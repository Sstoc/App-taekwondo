...
export default function LoginPage() {
  ...
  const { setUser, setProfile, showToast } = useAppStore()
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (profile) {
        setProfile(profile)
        navigate(profile.role === 'admin' ? '/admin/dashboard' : '/portal/status', { replace: true })
      } else {
        // Create default alumno profile
        const { data: newProfile } = await supabase
          .from('profiles')
          .upsert({ id: data.user.id, role: 'alumno' })
          .select()
          .single()
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
      ...
      >
        {/* Header — igual al original: bg-slate-900 */}
        <div className="p-8 bg-slate-900 text-white text-center">
          <img src="/logo chang moo kwan.jpeg" alt="Chang Moo Kwan"
            onError={e => {
        ...
        <div className="p-8 space-y-5">
          ...
          {error && (
            <p className="text-sm font-bold text-red-500 text-center bg-red-50 border border-red-200 py-3 px-2 rounded-xl">{error}</p>
          )}
          {info && (
            <p className="text-sm font-bold text-emerald-600 text-center bg-emerald-50 border border-emerald-200 py-3 px-2 rounded-xl">{info}</p>
          )}
